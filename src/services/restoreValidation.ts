...
export async function executeRestorePlan(repository:DataRepository,plan:RestorePlan,onBatch?:(module:RepositoryModule,batch:number,count:number)=>void){
 ...
 if(repository.kind==='supabase'){const{requireSupabase}=await import('../lib/supabase'),payload=Object.fromEntries(Object.entries(plan.records).filter(([module])=>module!=='deletion_logs'));const{data,error}=await requireSupabase().rpc('restore_backup_new_only',{p_operation_id:plan.operationId,p_file_hash:plan.fileHash,p_expected_inserts:plan.willInsert,p_records:payload,p_controlled_test:false});if(error)throw new Error(`Supabase: ${error.message}`);const result=data as{status?:string;inserted?:number;error?:string};if(result?.status!=='completed')throw new Error(result?.error||`Restauração ${result?.status||'não concluída'}.`);return{operationId:plan.operationId,status:'completed',inserted:result.inserted||0};}
