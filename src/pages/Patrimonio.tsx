...
export default function Patrimonio(){
  ...
  return <div className="space-y-6">
    ...
    <section className="grid gap-4 md:grid-cols-2">{assets.map(asset=><article key={`${asset.kind}-${asset.id}`} onClick={()=>navigate(`/bens/${asset.kind}/${asset.id}`)} className="cursor-pointer rounded-xl border bg-white p-5 custom-shadow"><div className="flex justify-between"><div><b>{asset.name||asset.model||asset.description||'Bem sem nome'} {asset.code&&<span className="text-primary">{asset.code}</span>}</b><p className="text-xs text-secondary">{asset.kind==='veiculo'?(asset.plate||'Placa não informada'):'Imóvel'} · {asset.status||'Status não informado'}</p></div>{asset.kind==='veiculo'?<Bike className="h-5 w-5"/>:<Building2 className="h-5 w-5"/>}</div><p className="mt-3 text-xs">Valor atual: <b>{brl(Number(asset.current_value||asset.purchase_value||0))}</b></p></article>)}</section>
  </div>;
}
