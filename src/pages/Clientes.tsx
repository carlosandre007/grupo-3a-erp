...
export default function Clientes(){
  ...
  return <div className="space-y-6">
    ...
    <section className="grid gap-4 md:grid-cols-2">{visible.map(client=><article key={client.id} className="rounded-xl border bg-white p-5 custom-shadow"><div className="flex justify-between gap-3"><div><b>{client.name||'Cliente sem nome'}</b><p className="text-xs text-secondary">{client.person_type||'Tipo não informado'} · {client.document||'Documento não informado'}</p></div><div className="flex gap-2"><button onClick={()=>navigate(`/clientes/${client.id}`)} title="Ver detalhes"><ExternalLink className="h-4 w-4"/></button><ProtectedRecordActions table="clients" record={client} fields={[{key:'name',label:'Nome'},{key:'email',label:'E-mail'},{key:'phone',label:'Telefone'},{key:'address',label:'Endereço'}]} onChanged={()=>setVersion(value=>value+1)}/></div></div><div className="mt-4 space-y-1 text-xs">{client.email&&<p><Mail className="mr-2 inline h-4 w-4"/>{client.email}</p>}{client.phone&&<p><Phone className="mr-2 inline h-4 w-4"/>{client.phone}</p>}<p><Building className="mr-2 inline h-4 w-4"/>{companyNames.get(String(client.company_id||''))||'Empresa não vinculada'}</p></div></article>)}</section>
  </div>;
}
